...
menu=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='👨‍🍳 ЛК ресторана',web_app=WebAppInfo(url=f"{REG_WEBAPP_URL}/index.html?api={BACKEND_URL}"))],[InlineKeyboardButton(text='🍽 Экран покупателя',web_app=WebAppInfo(url=f"{BUYER_WEBAPP_URL}/index.html?api={BACKEND_URL}"))]])
@dp.message(CommandStart())
async def start(m:Message):
    await m.answer('Foody: выберите действие',reply_markup=menu)
